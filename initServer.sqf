	...
	if (!("server" call Gemini_fnc_persistence_exists)) then
		{
			...
			switch (OPEX_param_dayTime) do
				{
					case 0	:	{[[2019, 7, 1, ceil (random 23), ceil (random 59)]] remoteExec ["setDate"]}; // random time
					case 1	:	{[[2019, 7, 1, 4, 45]] remoteExec ["setDate"]}; // morning
					case 2	:	{[[2019, 7, 1, 12, 0]] remoteExec ["setDate"]}; // noon
					case 3	:	{[[2019, 7, 1, 19, 0]] remoteExec ["setDate"]}; // evening
					case 4	:	{[[2019, 7, 1, 0, 0]] remoteExec ["setDate"]}; // midnight
				};
