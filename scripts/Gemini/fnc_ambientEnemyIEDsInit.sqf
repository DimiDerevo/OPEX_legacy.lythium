...
OPEX_IED_disarm =
	{
		...
		//[OPEX_friendly_side1, "Base", "STR_globalStats_updatePositive", globalStats_OPEX_IED_disarmed, globalStats] remoteExec ["Gemini_fnc_globalChat"];
	};
OPEX_IED_delete =
	{
				...
				[_IED] spawn
					{
						...
						if (alive _IED) then {[_IED] call OPEX_IED_explosion};
					};
		[_IED, _position] spawn OPEX_IED_delete;
	};
// REMOTE trigger - IED can explode if: (DESTROYED or DAMAGED) or (ANY FRIEND IS NEARBY) or (TRIGGERMAN is KILLED)
OPEX_IED_trigger_remote =
	{
		private ["_IED", "_position"];
		_IED = _this select 0;
		_position = _this select 1;
							...
							if (round (random 10) > 6) then
								{
									...
									[_assistant, _position] spawn OPEX_IED_delete;
								};
						};
				// CREATING TRIGGER
					// checking if IED is destroyed or damaged
								...
								do
									{
										...
										then
											{
												if (({_x distance2D (position _IED) < (random 15)} count (allPlayers - entities "HeadlessClient_F") > 0) && ((alive _triggerMan) && ("Gemini_deadManSwitch" in ((items _triggerMan) + (uniformItems _triggerMan) + (backpackItems _triggerMan) + (vestItems _triggerMan))))) then {sleep (random 3); if (alive _IED) then {[_IED] call OPEX_IED_explosion}};
												if ((!alive _triggerMan) && ("Gemini_deadManSwitch" in ((items _triggerMan) + (uniformItems _triggerMan) + (backpackItems _triggerMan) + (vestItems _triggerMan)))) then {sleep (random 3); if (alive _IED) then {[_IED] call OPEX_IED_explosion}};
											}
										else
											{
												if (({_x distance2D (position _IED) < (random 15)} count (allPlayers - entities "HeadlessClient_F") > 0) && (alive _triggerMan)) then {sleep (random 3); if (alive _IED) then {[_IED] call OPEX_IED_explosion}};
												if (!alive _triggerMan) then {sleep (random 3); if (alive _IED) then {[_IED] call OPEX_IED_explosion}};
											};
									};
							};
						...
						[_IED, _position] spawn
							{
								...
								if (((getMagazineCargo ((nearestObjects [getPos player, ["GroundWeaponHolder"], 5]) select 0)) select 0 select 0) in ["IEDLandBig_Remote_Mag", "IEDUrbanBig_Remote_Mag", "IEDUrbanSmall_Remote_Mag", "IEDLandSmall_Remote_Mag"]) then {call OPEX_IED_disarm};
							};

					// DELETING TRIGGERMAN WHEN USELESS
					[_triggerMan, _position] spawn OPEX_IED_delete;
			};
		[_IED, _position] spawn OPEX_IED_delete;
	};
// CELLPHONE trigger - IED can explode if: (DESTROYED or DAMAGED) or ((ANY FRIEND IS NEARBY) and (TRIGGERMAN is ALIVE))
OPEX_IED_trigger_cellphone =
	{
		private ["_IED", "_position"];
		_IED = _this select 0;
		_position = _this select 1;
							...
							do
								{
									sleep 0.5;
									if (isClass (configFile >> "CfgPatches" >> "Gemini_items"))
									then {if (({_x distance2D (position _IED) < (random 15)} count (allPlayers - entities "HeadlessClient_F") > 0) && ((alive _triggerMan) && (!(_triggerMan getVariable "isHandcuffed")) && ("Gemini_cellphone" in ((items _triggerMan) + (uniformItems _triggerMan) + (backpackItems _triggerMan) + (vestItems _triggerMan))))) then {sleep (random 10); if (alive _IED) then {[_IED] call OPEX_IED_explosion}}}
									else {if (({_x distance2D (position _IED) < (random 15)} count (allPlayers - entities "HeadlessClient_F") > 0) && ((alive _triggerMan) && (!(_triggerMan getVariable "isHandcuffed")))) then {sleep (random 5); if (alive _IED) then {[_IED] call OPEX_IED_explosion}}}
								};
						};
					// checking if IED is disarmed
						[_IED, _position] spawn
							{
								private ["_IED", "_IEDPosition"];
								_IED = _this select 0;
								private _IEDPos = position _IED;
								while {alive _IED} do {sleep 1};
								if (((getMagazineCargo ((nearestObjects [_IEDPos, ["GroundWeaponHolder"], 5]) select 0)) select 0 select 0) in ["IEDLandBig_Remote_Mag", "IEDUrbanBig_Remote_Mag", "IEDUrbanSmall_Remote_Mag", "IEDLandSmall_Remote_Mag"]) then {call OPEX_IED_disarm};
							};
						...
						[_IED] spawn
							{
								...
								do
									{
										sleep 0.5;
										if ({(_x distance2D (position _IED) < (random 5)) && ((speed _x > _maxSpeed) || (speed _x < _maxSpeed * -1))} count (allUnits) > 0) then {sleep (random 5); if (alive _IED) then {[_IED] call OPEX_IED_explosion}};
									};
							};
					// checking if IED is disarmed
						[_IED, _position] spawn
