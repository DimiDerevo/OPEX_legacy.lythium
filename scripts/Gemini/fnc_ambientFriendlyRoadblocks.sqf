...
private _mainFunction =
	{
		...
		if ((count (OPEX_playingPlayers select {speed _x < 20}) == 0) || (OPEX_ambientFriendlyRoadblock >= 1) || (random 100 > 90)) exitWith {};
		...
		[_squad, _roadPos] spawn
			{
				private _roadMarker = [_this#1, "mil_warning", "ColorBlue", "STR_marker_friendlyRadblock"] call Gemini_fnc_createMarker;
				while {count (units (_this#0)) > 0} do {sleep 30;};
				deleteMarker _roadMarker;
			};
		...
		if (OPEX_debug) then
			{
				...
				[_roadPos, "mil_warning", "ColorBlue", "FRIENDLY ROADBLOCK"] call Gemini_fnc_createMarker;
			};
