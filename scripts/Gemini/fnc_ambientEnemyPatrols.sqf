...
private _mainFunction =
	{
		...
		if (count OPEX_playingPlayers <= 5) then {_maxPatrols = 2};
		if ((count OPEX_playingPlayers > 5) && (count OPEX_playingPlayers <= 9)) then {_maxPatrols = 4};
		if (count OPEX_playingPlayers > 9) then {_maxPatrols = 6};

		// CHECKING IF CONDITIONS ARE VALID
		if ((count (OPEX_playingPlayers select {speed _x < 20}) == 0) || (OPEX_ambientEnemyPatrols >= _maxPatrols) || (random 100 > 50)) exitWith {};
		...
		// SPAWNING ENEMIES
		private _squad = [OPEX_enemy_side1, ["infantry", "infantry", "infantry", "infantry", "infantry", "infantry", "infantry", "infantry", "infantry", "infantry", "motorized", "motorized"], selectRandom [2,3,4], _player, [OPEX_spawnDistanceMini, OPEX_spawnDistanceMaxi], "patrol", _player, OPEX_enemy_AIskill, 100] call Gemini_fnc_spawnSquad;
