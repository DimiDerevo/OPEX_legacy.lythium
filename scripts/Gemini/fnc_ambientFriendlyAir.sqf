...
private _mainFunction =
	{
		...
		if ((count (OPEX_playingPlayers select {speed _x < 20}) == 0) || (OPEX_ambientFriendlyAir >= 1) || (random 100 > 60)) exitWith {};
		private _allPlayers = allPlayers - entities "HeadlessClient_F";
		if ((count (_allPlayers select {speed _x < 20}) == 0) || (OPEX_ambientFriendlyAir >= 1) || (random 100 > 60)) exitWith {};
		...
		private ["_type"];
		if (random 1 > 0.9) then {_type = "aircraft"} else {_type = "chopper"};
		...
		private _vehicle = ([_spawnPos, _azimuth, _class, west] call BIS_fnc_spawnVehicle) select 0;
