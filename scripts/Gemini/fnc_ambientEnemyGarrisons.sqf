...
private _mainFunction =
	{
		...
		if ((count (OPEX_playingPlayers select {speed _x < 20}) == 0) || (OPEX_ambientEnemyGarrisons >= 1) || (random 100 > 80)) exitWith {};
		...
		diag_log text format ["[AEG] Selected location pos: %1", _locationPos];

		// SPAWNING OBJECTS
		private _flagPos = ["land", _locationPos, 1, _locationSize / 2, 3] call Gemini_fnc_findPos;
		if (_flagPos isEqualTo [0,0,0]) exitWith {OPEX_ambientEnemyGarrisons = OPEX_ambientEnemyGarrisons - 1; publicVariable "OPEX_ambientEnemyGarrisons";};
		diag_log text format ["[AEG] Selected flag pos: %1", _flagPos];
