						...
						[_IED, _position] spawn
							{
								...
								while {alive _IED} do {sleep 1};
								if (((getMagazineCargo ((nearestObjects [_IEDPos, ["GroundWeaponHolder"], 5]) select 0)) select 0 select 0) in ["IEDLandBig_Remote_Mag", "IEDUrbanBig_Remote_Mag", "IEDUrbanSmall_Remote_Mag", "IEDLandSmall_Remote_Mag"]) then {call IED_disarm};
							};
...
private _mainFunction =
	{
		...
		if ((count (OPEX_playingPlayers select {speed _x < 20}) == 0) || (OPEX_ambientEnemyIEDs >= _maxIEDs) || (random 100 > 10)) exitWith {};
		...
		if (OPEX_debug) then
			{
				...
				switch _triggerType do
					{
						case "none"			:	{private _marker = [_roadPos, "mil_warning", "ColorRed", "IED (no trigger)"] call Gemini_fnc_createMarker;};
						case "remote"		:	{private _marker = [_roadPos, "mil_warning", "ColorRed", "IED (remote trigger)"] call Gemini_fnc_createMarker;};
						case "cellphone"	:	{private _marker = [_roadPos, "mil_warning", "ColorRed", "IED (cellphone trigger)"] call Gemini_fnc_createMarker;};
						case "proximity"	:	{private _marker = [_roadPos, "mil_warning", "ColorRed", "IED (proximity trigger)"] call Gemini_fnc_createMarker;};
						case default			{private _marker = [_roadPos, "mil_warning", "ColorRed", "IED (no trigger)"] call Gemini_fnc_createMarker;};
					};
			};
