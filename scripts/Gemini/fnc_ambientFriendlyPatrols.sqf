...
private _mainFunction =
	{
		...
		if ((count (OPEX_playingPlayers select {speed _x < 20}) == 0) || (OPEX_ambientFriendlyPatrols >= _maxPatrols) || (random 100 > 80)) exitWith {};
		...
		if ((OPEX_assignedTask) && (OPEX_taskID == "05")) exitWith {};
		...
		// 	{
		// 		while {count (units (_this select 0)) > 0} do {sleep 30/*if (OPEX_debug) then {sleep 3} else {sleep 30}; (_this select 1) setMarkerPos (position leader (_this select 0))*/};
